...
beforeEach( function () {
	$this->curl = new \T7\HTTP\Request();
	$this->curl->default_options['using'] = 'curl';
	$this->php = new \T7\HTTP\Request();
	$this->php->default_options['using'] = 'php';
} );

test( 'get-curl', function () {
	$response = $this->curl->get( url: 'http://localhost:17171/' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );
	expect( $response->headers['content-type'] )->toBe( 'application/json' );

	$response = $this->php->get( url: 'http://localhost:17171/' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );
	expect( $response->headers['content-type'] )->toBe( 'application/json' );
} );
...
test( 'get-404-response', function () {
	$response = $this->curl->get( url: 'http://localhost:17171/not-found' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );

	$response = $this->php->get( url: 'http://localhost:17171/not-found' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );
} );
...
test( 'get-invalid-url', function () {
	$response = $this->curl->get( url: 'http://invalid-domain-that-does-not-exist.test' );
	expect( $response->error )->toBe( true );
	expect( $response->code )->toBe( 0 );

	$response = $this->php->get( url: 'http://invalid-domain-that-does-not-exist.test' );
	expect( $response->error )->toBe( true );
	expect( $response->code )->toBe( 0 );
} );
...
test( 'get-redirect-handling', function () {
	$response = $this->curl->get( url: 'http://localhost:17171/redirect' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 302 );
	expect( $response->headers['location'] )->not->toBe( 'http://localhost:17171/redirect' );

	$response = $this->php->get( url: 'http://localhost:17171/redirect' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 302 );
	expect( $response->headers['location'] )->not->toBe( 'http://localhost:17171/redirect' );
} );
...
test( 'get-large-response', function () {
	$response = $this->curl->get( url: 'http://localhost:17171/large' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );
	expect( strlen( $response->body ) )->toBeGreaterThan( 1024 * 1024 ); // At least 1MB

	$response = $this->php->get( url: 'http://localhost:17171/large' );
	expect( $response->error )->toBe( false );
	expect( $response->code )->toBe( 200 );
	expect( strlen( $response->body ) )->toBeGreaterThan( 1024 * 1024 ); // At least 1MB
} );
